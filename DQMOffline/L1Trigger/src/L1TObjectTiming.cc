...
void L1TObjectTiming::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const&, edm::EventSetup const&){
  ...
  h_nMatched_ = 
      ibooker.book1D("BXOfMatchedJets", "Relative bunch crossing for matched L1 jets in unprefirable events; Relative Bunch Crossing; No. of events", 5, -2.5, 2.5);
  ...
  for (int i=0; i<kNumBx; ++i){
      ...
      std::string title = "E_{T} vs #eta for PFJets matched to L1Jet for BX=" + to_string(bx);
      title += "; PF Jet #eta; PF Jet E_{T} (GeV)";
      h_pt_eta_[i]=ibooker.book2D("PFJetEtVsEta_" + bx_name, title, 50, -5, 5, 35, 0, 700);

      title = "#phi vs #eta for PFJets matched to L1Jet for BX=" + to_string(bx);
      title += "; PF Jet #eta; PF Jet #phi";
      h_phi_eta_[i]=ibooker.book2D("PFJetPhiVsEta_" + bx_name, title, 50, -5, 5, 35, -3.2, 3.2);
  }
}
...
void L1TObjectTiming::analyze(edm::Event const& e, edm::EventSetup const& eSetup){
  ...
  for (auto pfjet = pfJets->begin(); pfjet != pfJets->end(); ++pfjet){
    ...
    if (foundMatch){
      fillWithMatchedJetPair(*matchedL1Jet, *pfjet, bxWithMatch);
    }
  }

}

void L1TObjectTiming::fillWithMatchedJetPair(const l1t::Jet& l1jet, const reco::PFJet& pfjet, int l1tBX){
